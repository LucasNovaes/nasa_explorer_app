...
void main() {
  group('Failure', () {
    test('two Failures with the same properties should be equal', () {
      const failure1 = Failure('Test Message', type: FailureType.unexpected);
      const failure2 = Failure('Test Message', type: FailureType.unexpected);

      expect(failure1, equals(failure2));
    });

    test('Failure objects with different properties should not be equal', () {
      const failure1 = Failure('Test Message 1', type: FailureType.unexpected);
      const failure2 = Failure('Test Message 2', type: FailureType.unexpected);
    ...
    test('Failure objects with different types should not be equal', () {
      const failure1 = Failure('Test Message', type: FailureType.unexpected);
      const failure2 = Failure('Test Message', type: FailureType.accessDenied);
    ...
    test('Failure objects with null exceptions should be equal', () {
      const failure1 = Failure('Test Message');
      const failure2 = Failure('Test Message');

      expect(failure1, equals(failure2));
    });

    test('Failure objects with different null exceptions should be equal', () {
      const failure1 = Failure('Test Message', exception: null);
      const failure2 = Failure('Test Message', exception: null);
